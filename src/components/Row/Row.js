            ...
            <div id={elementId} className={`row__posters  ${(isLargeRow ? 'posters' : 'thumbnails')}`}>
                {
                    loading && movies.length > 0 ?
                        movies.map((m, key) => (
                            isLargeRow ? key < 10 ? <Poster base_url={baseUrl} movie={m} count={++key} key={key} /> : null :
            ...
            </div>
            {
                movies.length > 0 ?
                    <div id="" className={`controls  ${(isLargeRow ? 'posters' : 'thumbnails')}`} >
                        {
                            scrollPostion > 0 ?
                                <button onClick={scrollLeft} className="left"> <i className="fa fa-angle-left"></i></button> : null

                        }
                        <button onClick={scrollRight} className="right"><i className="fa fa-angle-right"></i></button>
                    </div> : null
            }
        </div>
    );
